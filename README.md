...
- 🎛️ **基础调色** — 曝光 / 对比度 / 白平衡 / HSL / 曲线 / 颗粒 / 暗角
- 🌈 **滤镜** — 内置预设（暖色 / 冷色 / 黑白 / 鲜艳）+ `.cube` LUT 支持
...
1. **新建项目** — 左侧输入客片组名称
2. **导入照片** — 输入 RAW / JPEG / PNG 路径（逗号分隔），或导入到项目后点选
3. **下载模型** — 首次使用 AI 功能前，在右侧「模型下载」面板下载 4 个 ONNX 模型
4. **编辑** — 磨皮 / 美型 / 追色 / 祛瑕 / 调色 / 滤镜，编辑即自动预览
5. **导出** — 顶栏或右侧「导出」按钮输出 16bit TIFF
...
crates/
  ...
  storage/    SQLite 数据层
apps/desktop/
  src-tauri/  Tauri 后端（IPC 命令 / 模型下载 / 事件转发）
  src/        React 前端（面板 / 画布 / 菜单栏 / 状态栏）
scripts/      CI 图标生成
```
